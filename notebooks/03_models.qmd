...
Overall, all three models (Linear Regression, Random Forest, Gradient Boosting) reach almost perfect performance on the test set, with MAE and RMSE very close to zero and R² equal to 1.0. This happens because the features already capture the main relationships in the data in a smooth, almost linear way.
...
Although all models achieve extremely high accuracy, this may indicate that the engineered features encode the target too directly or that the underlying legacy system is deterministic. As a result, strong performance alone is not sufficient enough to be proven correct, and the interpretability checks are required to validate that the model's behavior aligns with business logic expectations. 
 
# Note on polynomial/interaction features

We briefly considered adding polynomial or interaction terms (for example squared mileage or interactions between miles and receipts). However, after feature engineering and scaling, all three models already achieved R^2 = 1.0 on the test set. This suggests that the existing features capture the relationship very well, so adding extra nonlinear terms would mainly increase complexity without improving performance.
